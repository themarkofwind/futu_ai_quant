...
# 默认开启；日K TTL 建议小于分析间隔（盘中默认 30 分钟）
KLINE_CACHE_ENABLED = os.getenv("KLINE_CACHE_ENABLED", "1").lower() not in ("0", "false", "no")
KLINE_CACHE_TTL_SEC = int(os.getenv("KLINE_CACHE_TTL_SEC", "1500"))
KLINE_WEEKLY_CACHE_TTL_SEC = int(os.getenv("KLINE_WEEKLY_CACHE_TTL_SEC", "14400"))
# 单轮分析内去重（不依赖磁盘缓存，始终生效）
KLINE_ROUND_CACHE_TTL_SEC = int(os.getenv("KLINE_ROUND_CACHE_TTL_SEC", "600"))
RSI_LENGTH = 14
BOLL_LENGTH = 20
BOLL_STD = 2
MACD_FAST = 12
